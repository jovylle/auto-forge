...
Return ONLY JSON: {"title":"...","category":"...","description":"...","features":["..."],"tags":["..."]}`;
    ...
    const pool = [
      ...
      { title: "Lantern Ledger", category: "tool", description: "A night journal lit one lantern at a time.", features: ["night entries","lantern glow","streak flame"], tags:["journal","lantern","localStorage"] },
      { title: "Comet Queue", category: "tool", description: "Your tasks as comets orbiting the urgent.", features: ["task orbits","gravity sort","impact alerts"], tags:["tasks","orbits","canvas"] },
      { title: "Brine Clock", category: "art", description: "A tide timer that breathes with the sea.", features: ["tide dial","breath sync","harbor bell"], tags:["tide","time","canvas"] },
      { title: "Paper Garden", category: "generative", description: "Fold origami flora that blooms on click.", features: ["fold engine","bloom garden","seed sharing"], tags:["origami","garden","generative"] },
      { title: "Static Picnic", category: "social", description: "Host a radio potluck for passing strangers.", features: ["dish tuner","picnic board","signal seats"], tags:["radio","social","audio"] },
      { title: "Ember Atlas", category: "viz", description: "Chart your week as a cooling heat map.", features: ["heat diary","cool-down trails","ember export"], tags:["heatmap","diary","canvas"] },
      { title: "Moon Pantry", category: "tool", description: "A lunar recipe box for midnight cooks.", features: ["recipe phases","pantry stock","midnight mode"], tags:["recipes","pantry","tool"] },
      { title: "Glass Orchard", category: "game", description: "Merge glowing fruit in a glass greenhouse.", features: ["merge board","greenhouse glow","harvest score"], tags:["merge","fruit","game"] },
    ];
...
export async function generateIdea() {
  ...
  let titleScore = 0;
  try { titleScore = dedupScore({ title: cand.title, tags: [], features: [] }, q.projects); } catch {}
  if (score > 0.62 || titleScore > 0.6) {
    ...
    let titleScore2 = 0;
    try { titleScore2 = dedupScore({ title: cand2.title, tags: [], features: [] }, q.projects); } catch {}
  ...
  if (score > 0.75 || titleScore > 0.6) {
    ...
    const freshSeeds = seeds.filter(s => dedupScore({ title: s.title, tags: s.tags||[], features: [] }, q.projects) < 0.75);
